...
def grocery_list(dish: str, servings: int) -> str:
    ...
    lines = [
        f"🛒 Grocery List for {dish.title()} ({servings} servings):"
    ]
...
def cost_calculator(dish: str, servings: int) -> str:
    ...
    cost = prices[dish_key] * (servings / 5)

    return (
        f"💰 Estimated Cost for {dish.title()} "
        f"({servings} servings): ₹{cost:,.0f}"
    )
...
def home():
    ...
    <body>
        ...
        <p>
            Ask for recipes, grocery ingredient lists,
            and approximate budget estimates.
        </p>

        <textarea
            id="msg"
            placeholder="e.g. Find me a recipe for chicken biryani for 10 people with grocery list and total cost."
        ></textarea>
